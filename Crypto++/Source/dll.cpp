...
static void SetNewAndDeleteFunctionPointers()
{
	...
	hModule = GetModuleHandle(_T("msvcrtd"));
	if (!hModule)
		hModule = GetModuleHandle(_T("msvcrt"));
	if (hModule)
	{
		// 32-bit versions
	...
	OutputDebugString(_T("Crypto++ was not able to obtain new and delete function pointers.\n"));
	throw 0;
}
