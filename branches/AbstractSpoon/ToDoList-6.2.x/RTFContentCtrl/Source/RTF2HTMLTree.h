...
//	Node node;
	Node Parent()
	{
		if (!NodeBase::get_Data().IsNull())
		{
			return NodeBase::get_Data().Parent;
		}
		return *this;
	};
	operator Type&()
	{
		return (NodeBase::get_Data().tData.get_Data());
	};
	Type* operator->()
	{
		return &(NodeBase::get_Data().tData.get_Data());
	...
	Ref<Type> get_Data()
	{
		return NodeBase::get_Data().tData;
	};
	...
	bool IsRoot()
	{
		return NodeBase::get_Data().Parent.IsNull();
	};
	...
	{
		return (Node)NodeBase::get_Data().vChilds[nIndex];
	};
	__declspec(property(get = get_Count)) int Count;
	int get_Count()
	{
		return NodeBase::get_Data().vChilds.size();
	};
	...
	{
		Node n(t);
		((NodeBase&)n).get_Data().Parent = (*this);
		NodeBase::get_Data().vChilds.push_back(n);
		return n;
	};
	void Delete()
	{
		if (!NodeBase::get_Data().Parent->IsNull())
		{
			NodeBase::get_Data().Parent->DeleteNode(*this);
		}
		//Release();
	};
