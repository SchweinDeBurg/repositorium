...
struct TDCSTARTUP
{
	...
	{
		_tcscpy(szFilePath, startup.szFilePath);
...
#define COPYTEXT(DEST, SRC) _tcsncpy(DEST, SRC, min(sizeof(DEST) - 1, SRC.GetLength() + 1))
	...
	TDCSTARTUP(const CEnCommandLineInfo& cmdInfo) : dwIDSel(0), dwPid(0), dwFlags(TLD_PASSWORDPROMPTING)
	{
		...
		// filepath
		COPYTEXT(szFilePath, cmdInfo.m_strFileName);
		...
		{
			dwFlags |= TLD_NEWTASK;
			COPYTEXT(szNewTask, sValue);

			// user can specify parentID else 0
			if (cmdInfo.GetOption(_T("pid"), sValue))
			{
				dwPid = _ttoi(sValue);
			}
		}
		...
		{
			COPYTEXT(szFilePath, sValue);
			dwFlags |= TLD_IMPORTFILE;
		}

		// external ID
		if (cmdInfo.GetOption(_T("xid"), sValue))
		{
			COPYTEXT(szExternalID, sValue);
		}
		...
		{
			sValue.Replace(_T("\\n"), _T("\n"));
			COPYTEXT(szComments, sValue);
		}

		// flags
		if (cmdInfo.HasOption(_T("v")))
		{
			dwFlags |= TLD_FORCEVISIBLE;
		}

		if (cmdInfo.HasOption(_T("x")))
		{
			dwFlags &= ~TLD_PASSWORDPROMPTING;
		}

		if (cmdInfo.HasOption(_T("l")))
		{
			dwFlags |= TLD_LOGGING;
		}
	}
	...
	TCHAR szFilePath[MAX_PATH]; // tasklist to load/import
	DWORD dwIDSel; // task to select
	DWORD dwPid; // parent task for new subtask
	TCHAR szNewTask[256], szComments[1024]; // task title/comments
	TCHAR szExternalID[256];
	DWORD dwFlags;
};
...
static TDC_ATTRIBUTE MapColumnToAttribute(TDC_COLUMN col)
{
	...
	{
	case TDCC_PRIORITY:
		return TDCA_PRIORITY;
	case TDCC_PERCENT:
		return TDCA_PERCENT;
	case TDCC_TIMEEST:
		return TDCA_TIMEEST;
	case TDCC_TIMESPENT:
		return TDCA_TIMESPENT;
	case TDCC_STARTDATE:
	...
	case TDCC_CREATEDBY:
		return TDCA_CREATEDBY;
	case TDCC_LASTMOD:
		return TDCA_LASTMOD;
	case TDCC_RISK:
		return TDCA_RISK;
	case TDCC_EXTERNALID:
		return TDCA_EXTERNALID;
	case TDCC_COST:
		return TDCA_COST;
	case TDCC_DEPENDENCY:
		return TDCA_DEPENDENCY;
	case TDCC_RECURRENCE:
		return TDCA_RECURRENCE;
	case TDCC_VERSION:
		return TDCA_VERSION;
	case TDCC_ICON:
		return TDCA_ICON;
...
static TDC_COLUMN MapSortByToColumn(TDC_SORTBY nSortBy)
{
	...
	case TDC_SORTBYRECENTEDIT:
		return TDCC_RECENTEDIT;
	case TDC_SORTBYREMAINING:
		return TDCC_REMAINING;
	case TDC_SORTBYDONE:
		return TDCC_DONE;
	case TDC_SORTBYICON:
		return TDCC_ICON;
...
struct SEARCHPARAMS
{
	...
	{
		int nRule = rules.GetSize();

		while (nRule--)
		{
			if (rules[nRule].attrib == attrib)
			{
				return TRUE;
			}
