...
int CCalendarUtils::CompareDates(const COleDateTime& _dt1, const COleDateTime& _dt2)
{
	int nReturn = 0;
...
{
	if ((_dt.GetYear() > DATE_RANGE_YEAR_MIN) && (_dt.GetYear() < DATE_RANGE_YEAR_MAX))
	{
		return TRUE;
	}
	return FALSE;
...
CString CCalendarUtils::GetWeekday(int _iDayOfWeek, BOOL _bShort)
{
	CString strReturn;
	if ((_iDayOfWeek >= 0) && (_iDayOfWeek < 7))
	{
		...
		int iFirstDayOfWeek = CDateHelper::FirstDayOfWeek();
		...
		strReturn = CDateHelper::GetWeekday(nDay, _bShort);
	}
	else
	{
		ASSERT(FALSE);
...
void CCalendarUtils::GetWeekdays(CStringList& _listDays, BOOL _bShort, BOOL _bWeekendsHidden)
{
	_listDays.RemoveAll();

	int nCurrentDay = CDateHelper::FirstDayOfWeek();	//start with the first day of the week
	...
	for (int i = 0; i < 7; i++)
	{
		...
		{
			CString strDay = CDateHelper::GetWeekday(nCurrentDay, _bShort);
