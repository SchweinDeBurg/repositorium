                               ...
                               m_bDoExpandoAnimation(TRUE)
{
  ...
  if (GetVersionEx(&osvi) && osvi.dwMajorVersion >= 6) //If we're running on Vista, then use the user preference for "SPI_GETCLIENTAREAANIMATION" by default
...
HRESULT CXTaskDialog::Layout()
{
  ...
  m_dlgItems.RemoveAll();  
	...
	if (m_hMainIcon)
	{
		...
		rDialog.right += (32 + SpacingSize);

    //Add the main icon to the template	
		rMainIcon = CRect(nLeftText, MainIconVerticalOffset, nLeftText + 32, MainIconVerticalOffset + 32);
...
HRESULT CXTaskDialog::TaskDialogIndirect(const TASKDIALOGCONFIG* pTaskConfig, int* pnButton, int* pnRadioButton, BOOL* pfVerificationFlagChecked)
{
  ...
  if (FAILED(hr))
  {
    ATLTRACE(_T("CXTaskDialog::TaskDialogIndirect, Failed to load up main window title\n"));
    return hr;
  }
...
LRESULT CXTaskDialog::OnCommand(UINT /*nMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
  ...
  {
    ATLASSUME(m_pTaskConfig->pRadioButtons);
