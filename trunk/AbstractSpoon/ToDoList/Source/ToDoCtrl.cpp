...
void CToDoCtrl::LoadTaskIcons()
{
	if (m_ilTaskIcons.Create(16, 16, ILC_COLOR32 | ILC_MASK, 0, 200))
	{
		...
		CString sSearchPath = FileMisc::GetModuleFolder() + _T("Resources\\TaskIcons.bmp");
		CEnBitmap ebm;
		...
		CFileFind ff;
		sSearchPath = FileMisc::GetModuleFolder() + _T("Resources\\*.ico");
	...
	case TDCC_COST:
		{
			...
			if (dMaxCost > 0 || !HasStyle(TDCS_HIDEZEROTIMECOST))
				nWidth = GraphicsMisc::GetTextWidth(pNCRC->pDC, _T("%.2f"), dMaxCost);
