...
BOOL CToDoCtrl::OnInitDialog() 
{
	...
	for (int nCol = 0; nCol < NUM_COLUMNS; nCol++)
	{
		...
		{
			m_tree.AddGutterColumn(tdcc.nColID, " ", 0, tdcc.nAlignment);
...
void CToDoCtrl::LoadTaskIcons()
{
	if (m_ilTaskIcons.Create(16, 16, ILC_COLOR32 | ILC_MASK, 0, 200))
	{
		...
		CString sSearchPath = FileMisc::GetModuleFolder() + "Resources\\TaskIcons.bmp";
		CEnBitmap ebm;
		...
		CFileFind ff;
		sSearchPath = FileMisc::GetModuleFolder() + "Resources\\*.ico";
...
BOOL CToDoCtrl::CreateContentControl(BOOL bResendComments)
{
	...
	if (bSuccess)
	{
		...
		// set where preferences are to be loaded from
		CString sPrefKey = GetPreferencesKey(m_cfComments);
		m_ctrlComments.SetPreferenceLocation(sPrefKey);
	}
...
{
	pDWM->MoveWindow(GetDlgItem(IDC_TASKLIST), rPos);
}
...
void CToDoCtrl::ReposComments(int cx, int cy, CDeferWndMove* pDWM, CRect& rComments /*out*/) 
{
	...
	CRect rComField(1, 0, cx, cy); // init x to 1 else MoveCtrl won't do anything
...
void CToDoCtrl::UpdateSelectedTaskPath()
{
	...
	if (HasStyle(TDCS_SHOWPATHINHEADER))
	{
		...
		if (GetSelectedCount() == 1)
		{
			...
			if (!sPath.IsEmpty())
				sHeader.Format("%s [%s]", CEnString(IDS_TDC_COLUMN_TASK), sPath);
		}
...
{
	// don't update whilst the user has their finger down on a cursor key
// 	if (Misc::IsAnyCursorKeyPressed())
// 		return;

// #ifdef _DEBUG
// 	static int nUpdate = 1;
// 	TRACE("CToDoCtrl::UpdateControls(%d)\n", nUpdate++);
// #endif
...
//	BOOL bEditDue = (nSelCount >= 1);
	...
	BOOL bHideColor = HasStyle(TDCS_COLORTEXTBYCATEGORY) || HasStyle(TDCS_COLORTEXTBYPRIORITY) ||
						HasStyle(TDCS_COLORTEXTBYNONE);
	BOOL bHasDueDate = FALSE, bHasStartDate = FALSE, bHasDoneDate = FALSE;
	...
	if (hti)
	{
		...
		bHasDueDate = SelectionHasDates(TDCD_DUE);
		...
		bHasStartDate = SelectionHasDates(TDCD_START);
		...
		bHasDoneDate = SelectionHasDates(TDCD_DONE);
	...
	else // clear controls
	{
		...
		m_sVersion.Empty();
		m_crColour = CLR_DEFAULT;//GetSysColor(COLOR_WINDOWTEXT);
...
//			break;
	...
	BOOL bHide = (HasStyle(TDCS_VERTCOMMENTS) || (m_nMaxState != TDCMS_NORMAL));
	...
	case TDCA_DEPENDENCY:
		{
			...
			CStringArray aDepends;
			Misc::ParseIntoArray(m_sDepends, aDepends);
...
BOOL CToDoCtrl::SetSelectedTaskComments(LPCTSTR szComments, const CString& sCustomComments, BOOL bInternal)
{
	...
	LPCSTR szFormat = bCanChangeCustom ? m_cfComments : NULL;
...
BOOL CToDoCtrl::SetSelectedTaskDate(TDC_DATE nDate, const COleDateTime& date)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		case TDCD_STARTTIME:
		case TDCD_STARTDATE:SetModified(TRUE, TDCA_STARTDATE); break;
		case TDCD_DUETIME:
		case TDCD_DUEDATE:	SetModified(TRUE, TDCA_DUEDATE); break;
		case TDCD_DONETIME:	SetModified(TRUE, TDCA_DONEDATE); break;
...
BOOL CToDoCtrl::OffsetSelectedTaskDate(TDC_DATE nDate, int nAmount, int nUnits, BOOL bAndSubtasks)
{
	...
	while (pos)
	{
		...
		int nItemRes = m_data.OffsetTaskDate(m_find.GetTaskID(hti), nDate, nAmount, nUnits, bAndSubtasks);
...
BOOL CToDoCtrl::SetSelectedTaskDone(const COleDateTime& date)
{
	...
	// check for multiple recurring tasks
/*
	if (bDone && GetSelectionRecurringTaskCount() > 1)
	{
		AfxMessageBox(IDS_CANTCOMPLETEMULTIPLERECURRINGTASKS);
		return FALSE;
	}
*/
...
int CToDoCtrl::SetTaskDone(HTREEITEM hti, const COleDateTime& date, TDC_SETTASKDONE nSetChildren)
{
	...
	if (bDone && !bWasDone && HasStyle(TDCS_AUTOADJUSTDEPENDENTS))
		...
		if (nItemRes == SET_CHANGE)
      	{
			...
			if (HasStyle(TDCS_AUTOADJUSTDEPENDENTS))
			{
				...
				{
         			DWORD dwIDDepends = atoi(aDepends[nDepends]);
...
BOOL CToDoCtrl::GetInsertLocation(TDC_INSERTWHERE nWhere, HTREEITEM& htiDest, HTREEITEM& htiDestAfter)
{
	...
	else // determine the actual pos to insert
	{
		...
		case TDC_INSERTAFTERSELTASK:
			...
			// don't need this test because htiDestAfter must be non-NULL if it is equal to htiSel
// 			if (!htiDestAfter)
// 				htiDestAfter = TVI_LAST;
			break;
...
HTREEITEM CToDoCtrl::NewTask(LPCTSTR szText, TDC_INSERTWHERE nWhere, 
							 BOOL bEditText)
{
	if (IsReadOnly())
		return NULL;
...
LRESULT CToDoCtrl::OnEditEnd(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (!sText.IsEmpty())
	{
		...
		nRes = m_data.SetTaskTitle(m_dwEditingID, sText);
...
BOOL CToDoCtrl::SetStyles(const CTDCStyles& styles)
{
...
TDC_FILE CToDoCtrl::Save(CTaskFile& tasks/*out*/, LPCTSTR szFilePath)
{
	...
	if (tasks.Save(szFilePath))
	{
		...
		// update preferences in comments control
		CString sPrefKey = GetPreferencesKey(m_cfComments, szFilePath);
		m_ctrlComments.SetPreferenceLocation(sPrefKey);
...
BOOL CToDoCtrl::ModNeedsResort(TDC_ATTRIBUTE nModType, TDC_SORTBY nSortBy) const
{
	if (IsSortable(nSortBy))
	{
		switch (nModType)
		{
		case TDCA_TASKNAME:		return (TDC_SORTBYNAME == nSortBy);
		case TDCA_STARTDATE:	return (TDC_SORTBYSTARTDATE == nSortBy);
		case TDCA_PRIORITY:		return (TDC_SORTBYPRIORITY == nSortBy);
		case TDCA_COLOR:		return (TDC_SORTBYCOLOR == nSortBy);
		case TDCA_ALLOCTO:		return (TDC_SORTBYALLOCTO == nSortBy);
		case TDCA_ALLOCBY:		return (TDC_SORTBYALLOCBY == nSortBy);
		case TDCA_PERCENT:		return (TDC_SORTBYPERCENT == nSortBy);
		case TDCA_STATUS:		return (TDC_SORTBYSTATUS == nSortBy);
		case TDCA_CATEGORY:		return (TDC_SORTBYCATEGORY == nSortBy);
		case TDCA_TIMEEST:		return (TDC_SORTBYTIMEEST == nSortBy);
		case TDCA_RISK:			return (TDC_SORTBYRISK == nSortBy);
		case TDCA_EXTERNALID:	return (TDC_SORTBYEXTERNALID == nSortBy);
		case TDCA_TIMESPENT:	return (TDC_SORTBYTIMESPENT == nSortBy);
		case TDCA_VERSION:		return (TDC_SORTBYVERSION == nSortBy);
		case TDCA_RECURRENCE:	return (TDC_SORTBYRECURRENCE == nSortBy);
		case TDCA_ICON:			return (TDC_SORTBYICON == nSortBy);
					...
					case ID_TDD_ADDTASKDEPENDENCY:
						{
							...
							aDepends.Add(CEnString("%d", GetSelectedTaskID()));
...
void CToDoCtrl::PrepareTaskIDsForPaste(CTaskFile& tasks, TDC_RESETIDS nResetID) const
{
	...
	DWORD dwNextID(m_dwNextUniqueID);
...
#ifdef _DEBUG
	tasks.Save("PrepareTaskIDsForPaste.txt");
#endif
}
										...
										CMapID2ID& mapID, TDC_RESETIDS nResetID) const
{
	...
	if (nResetID == TDCR_YES || m_data.GetTask(dwTaskID) != NULL)
...
void CToDoCtrl::GetItemColors(DWORD dwTaskID, NCGITEMCOLORS* pColors, TDI_STATE nState)
{
	...
	case TDIS_SELECTEDNOTFOCUSED:
		if (!pColors->bBackSet)
		{
			...
			pColors->crBack = GraphicsMisc::Lighter(color, 0.33);
			pColors->bBackSet = TRUE;
		}
...
BOOL CToDoCtrl::DrawItem(const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, const NCGDRAWITEM* pNCGDI, TDI_STATE nState)
{
	...
	case TDCC_RECURRENCE:
		if (pTDI->trRecurrence.nRegularity != TDIR_ONCE)
		{
			CString sRecurrence = CTDLRecurringTaskEdit::GetRegularity(pTDI->trRecurrence.nRegularity);
	...
	case TDCC_RECENTEDIT:
		if (pTDI->IsRecentlyEdited())
			DrawGutterItemText(pNCGDI->pDC, "*", rItem, DT_CENTER);
		break;
	...
	case TDCC_TRACKTIME:
		if (m_dwTimeTrackTaskID == dwTaskID)
		{
			...
			if (font.GetSafeHandle())
			{
				...
				DrawGutterItemText(pNCGDI->pDC, "T", rItem, pNCGDI->nTextAlign, crText);
		}
		break;
	...
	case TDCC_FLAG:
		if (pTDI->bFlagged)
		{
			...
			{
				CFont* pOldFont = pNCGDI->pDC->SelectObject(&font);
				DrawGutterItemText(pNCGDI->pDC, "O", rItem, pNCGDI->nTextAlign);
				pNCGDI->pDC->SelectObject(pOldFont);
			}
			else
				DrawGutterItemText(pNCGDI->pDC, "F", rItem, pNCGDI->nTextAlign);
		}
		break;
	...
	case TDCC_REMINDER:
		if (GetParent()->SendMessage(WM_TDCM_TASKHASREMINDER, dwTaskID, (LPARAM)this))
		{
			...
			if (font.GetSafeHandle())
			{
				...
				DrawGutterItemText(pNCGDI->pDC, "R", rItem, pNCGDI->nTextAlign);
		}
		break;
	...
	case TDCC_REMAINING:
		{
			COleDateTime date = m_data.GetEarliestDueDate(pTDI, pTDS, HasStyle(TDCS_USEEARLIESTDUEDATE));
			
			if (date.m_dt > 0) 
			{
				double dRemaining = TODOITEM::GetRemainingDueTime(date);
				CString sTime = CTimeHelper(24.0, 7.0).FormatTimeHMS(dRemaining, THU_DAYS, TRUE);
...
BOOL CToDoCtrl::PasteTasks(TDC_PASTE nWhere)
{
	...
	if (AddTasksToTree(tasks, htiDest, htiDestAfter, nResetID))
   {
...
LRESULT CToDoCtrl::OnGutterNotifyHeaderClick(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	case TDCC_ICON:			nSortBy = TDC_SORTBYICON;			break;
...
LRESULT CToDoCtrl::OnGutterRecalcColWidth(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	{
	case TDCC_ID:
		pNCRC->nWidth = GraphicsMisc::GetTextWidth(pNCRC->pDC, "%u", m_dwNextUniqueID - 1);
		break; 
	...
	case TDCC_COST:
		{
			...
			if (dMaxCost > 0 || !HasStyle(TDCS_HIDEZEROTIMECOST))
				nWidth = GraphicsMisc::GetTextWidth(pNCRC->pDC, "%.2f", dMaxCost);
								  ...
								  HTASKITEM hTask, const TDCGETTASKS& filter, int nPos, BOOL bTitleCommentsOnly) const
{
   ...
   if (!(bTitleOnly || bTitleCommentsOnly))
   {
      ...
      if (pTDI->trRecurrence.nRegularity != TDIR_ONCE && filter.WantAttribute(TDCA_RECURRENCE))
         file.SetTaskRecurrence(hTask, pTDI->trRecurrence, 
         CTDLRecurringTaskEdit::GetRegularity(pTDI->trRecurrence.nRegularity));
...
void CToDoCtrl::IncrementTrackedTime()
{
	...
	if (m_dwTimeTrackTaskID == GetSelectedTaskID() && IsTaskLabelEditing())
		return;
...
void CToDoCtrl::SortTreeItem(HTREEITEM hti, const TDSORTPARAMS& ss, BOOL bMulti)
{
	...
	TVSORTCB tvs = { hti, bMulti ? CToDoCtrlData::CompareFuncMulti : CToDoCtrlData::CompareFunc, (LPARAM)&ss };
