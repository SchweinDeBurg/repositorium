...
protected:
	...
	inline BOOL IsCaptured()
	{
		return m_iState >= CAPTURED;
	}
	inline BOOL IsSourceWnd(HWND hwnd)
	{
		return (GetWindowType(hwnd) & DDW_SOURCE);
	}
	inline BOOL IsTargetWnd(HWND hwnd)
	{
		return (GetWindowType(hwnd) & DDW_TARGET);
	}
