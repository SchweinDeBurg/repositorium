 ...
 *		- 21/20/2008: Fixed a bug, the axis label weren't displayed anymore.
 *
 */
...
void CChartAxis::Draw(CDC *pDC)
{
	...
	NewFont.CreatePointFont(m_nFontSize,ATL::CA2T(m_strFontName.c_str()),pDC) ;
	...
	case atDateTime:
		{
			...
			strLabel = tickTime.Format(ATL::CA2T(m_strDTTickFormat.c_str()));
		}
		break;
	}
...
void CChartAxis::RefreshDTTickFormat()
{
	...
	{
	case tiSecond:
		m_strDTTickFormat = "%H:%M:%S";
		break;
	case tiMinute:
		m_strDTTickFormat = "%H:%M";
		break;
	case tiHour:
		m_strDTTickFormat = "%H:00";
		break;
	case tiDay:
		m_strDTTickFormat = "%d %b";
		break;
	case tiMonth:
		m_strDTTickFormat = "%b %Y";
		break;
	case tiYear:
		m_strDTTickFormat = "%Y";
		break;
