...
#define ToolTip_EnumTools(hWnd, i, pTI) ::SendMessage(hWnd, TTM_ENUMTOOLS, i, (LPARAM)pTI)
...
VOID Grid_OnDrawItem(HWND /*hwnd*/, const DRAWITEMSTRUCT *lpDIS)
{
	...
	if (PIT_CATALOG == pItem->iItemType)
	{
		...
		RECT rcText = { rectCatPart1.left + 6, rectCatPart1.top + 3, rectCatPart1.right - 6, rectCatPart1.bottom + 3 };
		DrawText(lpDIS->hDC, pItem->lpszCatalog, _tcslen(pItem->lpszCatalog), &rcText,
...
#endif
				_stprintf(buf, szFmt, pgfi.logFont.lfFaceName, PointSize);
				RECT rcText = { rectPart3.left + 3, rectPart3.top + 3, rectPart3.right + 3, rectPart3.bottom + 3 };
				DrawText(lpDIS->hDC, buf, _tcslen(buf), &rcText, DT_NOCLIP | DT_LEFT | DT_SINGLELINE);
