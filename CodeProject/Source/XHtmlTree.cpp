// XHtmlTree.cpp  Version 1.5 - article available at www.codeproject.com
//
		...
		if (IsLeftButtonUp())
		{
			...
			if (m_bDragging)				// case 1:  user is dragging
			{
					...
					(!IsChildNodeOf(hItem, m_hItemButtonDown)))
				{
					...
					else if ((hNewParent == 0) || 
							 (ItemHasChildren(hAfter) && IsExpanded(hAfter)))
					{
						hNewParent = hAfter;
						hAfter = TVI_FIRST;
					}
			...
			if (hItem && (hItem == hItemSelected))	// case 2:  user wants to edit a label
			{
...
//=============================================================================
{
	...
	if (pXTCD)
	{
		if (m_bSmartCheck && (pXTCD->nChildren != 0))
		{
			if (pXTCD->nChecked == 0)
				nState = UNCHECKED;
			else if (pXTCD->nChecked == pXTCD->nChildren)
				nState = CHECKED;
			else
				nState = TRISTATE;
		}
...
//=============================================================================
{
	...
	if (pXTCD)
	{
		...
		if (pXTCD->nChecked == pXTCD->nChildren)
			pXTCD->bChecked = TRUE;
		else
...
//=============================================================================
void CXHtmlTree::OnLButtonUp(UINT nFlags, CPoint point) 
//=============================================================================
{
...
//=============================================================================
{
	...
	if (pElement)
	{
		...
		if (!strName.IsEmpty() && strName.Compare(_T("...")) != 0)
		{
			...
			TRACE(_T("bChecked=%d  bEnabled=%d\n"), xhtd.bChecked, xhtd.bEnabled);
			...
			TVINSERTSTRUCT tvis      = { 0 };
			tvis.item.mask           = TVIF_TEXT;
			tvis.item.pszText        = (LPTSTR)(LPCTSTR) strName;
...
//=============================================================================
{
	TRACE(_T("in CXHtmlTree::OnBeginlabeledit\n"));
