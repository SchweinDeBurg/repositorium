...
void CZipStorage::Open(LPCTSTR lpszPathName, int iMode, ZIP_SIZE_TYPE uVolumeSize)
{
	...
	else // open existing
	{
		...
		OpenFile(lpszPathName, CZipFile::modeNoTruncate | (readOnly ? CZipFile::modeRead : CZipFile::modeReadWrite));
...
void CZipStorage::ChangeVolume(ZIP_VOLUME_TYPE uNumber)
{
	...
	OpenFile(IsSpanned() ? ChangeSpannedRead() : ChangeSplitRead(), CZipFile::modeNoTruncate | CZipFile::modeRead);
}
