...
#define SZ_UTIL_TAB            "TAB"
#define SZ_UTIL_UXTHEME        _T("UxTheme.dll")
...
void CXPTabCtrl::InitImageList(UINT nBitmapID)
{
	...
	{
		ASSERT(FALSE);   // tab control has to be created first and image list can be created only once
		return;
	}
...
//	BOOL bLeftTab=!bBottom && bVertic && !bBody;
	...
	if (bBody)
	{
		DrawThemesPart(dcMem.GetSafeHdc(), 9, 0, SZ_UTIL_TAB, &rcMem);   // TABP_PANE=9,  0, 'TAB'
	}
	else
	{
		DrawThemesPart(dcMem.GetSafeHdc(), 1, bSel ? 3 : (bHot ? 2 : 1), SZ_UTIL_TAB, &rcMem);
	}
	...
	{
		nStart = 3, nLenSub = 4;   // if bottom oriented flip the body contest only (no shadows were flipped)
	}
...
#define __USE_MASM__   // the same processing is in C and in asm. To use C -> comment the beginning of this line
		...
		bmpMem.DeleteObject();   // recreate rotated bitmap
		bmpMem.CreateCompatibleBitmap(pDC, szBmp.cx, szBmp.cy);
		dcMem.SelectObject(&bmpMem);
		bihOut.biWidth = szBmp.cx;
		bihOut.biHeight = bBody ? -szBmp.cy : szBmp.cy;
...
BOOL CXPTabCtrl::OnTabSelChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (m_ixSelOld >= 0 && m_ixSelOld != GetCurSel()  && IsExtendedTabThemedXP())   // else
	{
		...
		{
			CRect rcOldSel;
			GetItemRect(m_ixSelOld, rcOldSel);
...
BOOL IsThemeActiveEx()
{
	...
	HINSTANCE hDll = LoadLibrary(SZ_UTIL_UXTHEME);   // 'UxTheme.dll'
	if (hDll == NULL)
	{
		return FALSE;   // the DLL won't be available on anything except Windows XP
	}
	UINT(PASCAL * pfnIsThemeActive)();
	(FARPROC&)pfnIsThemeActive = GetProcAddress(hDll, SZ_UTIL_THEMEACT);   // 'IsThemeActive'
...
BOOL IsWinXP()
{
	return GetWinVersion() >= PACKVERSION(5, 1) ? TRUE : FALSE;
}
//----------------------------------------------------------------------------------------------------------
BOOL IsThemeActiveXP()
{
	return (IsWinXP()  && IsThemeActiveEx()) ? TRUE : FALSE;
}
//----------------------------------------------------------------------------------------------------------
...
HRESULT DrawThemesPart(HDC hDC, int iPartId, int iStateId, LPCSTR uiPartNameID, LPRECT prcBox)
{
	...
	{
		CStringA sPartName(uiPartNameID);
		if (sPartName.GetLength() > 0)
		{
			...
			(FARPROC&)pfnOpenThemeData = GetProcAddress(hDll, SZ_UTIL_THEMEOPN); // 'OpenThemeData'
			if (pfnOpenThemeData && (hTheme = pfnOpenThemeData(NULL, swPartName)) != 0)
			{
				...
				(FARPROC&)pfnDrawThemeBackground = GetProcAddress(hDll, SZ_UTIL_THEMEBCKG);   // 'DrawThemeBackground'
