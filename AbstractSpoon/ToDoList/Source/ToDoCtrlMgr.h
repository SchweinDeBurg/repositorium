...
public:
	...
	{
		return m_aToDoCtrls.GetSize();
	}
	inline int GetSelToDoCtrl() const
	{
	...
	void ClearWebDetails(int nIndex)
	{
		SetWebDetails(nIndex, _T(""), _T(""), _T(""), _T(""));
	}
...
protected:
	struct TDCITEM
	{
		...
		inline TDCM_PATHTYPE GetPathType() const
		{
			return nPathType;
		}
		...
		BOOL IsWebTaskList() const
		{
			return !sUrlPath.IsEmpty();
		}
		CString GetUrl() const
		{
			return sWebServer + sUrlPath;
		}
