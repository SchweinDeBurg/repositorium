		...
		if (s_plugins) {
			...
			*/
			...
			if (!load_local_plugins_only) {
				int count = 0;
				TCHAR buffer[MAX_PATH + 200];
				TCHAR current_dir[2 * _MAX_PATH], module[2 * _MAX_PATH];
				BOOL bOk = FALSE;
				...
				if (GetCurrentDirectory(2 * _MAX_PATH, current_dir) != 0) {
					if (GetModuleFileName(NULL, module, 2 * _MAX_PATH) != 0) {
						TCHAR *last_point = _tcsrchr(module, _T('\\'));
				...
				while (count < s_search_list_size) {
					...
					_tcscpy(buffer, ATL::CA2T(s_search_list[count]));
					_tcscat(buffer, _T("*.fip"));

					if ((find_handle = (long)_tfindfirst(buffer, &find_data)) != -1L) {
						do {
							_tcscpy(buffer, ATL::CA2T(s_search_list[count]));
							_tcsncat(buffer, find_data.name, MAX_PATH + 200);
						...
						} while (_tfindnext(find_handle, &find_data) != -1L);
...
FreeImage_RegisterExternalPlugin(const char *path, const char *format, const char *description, const char *extension, const char *regexpr) {
	if (path != NULL) {
		HINSTANCE instance = LoadLibrary(ATL::CA2T(path));
